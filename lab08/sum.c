...
int sum_vectorized( int n, int *a )
{
    ...
    __m128i elem_ld = _mm_setzero_si128( );
    __m128i sum_vector = _mm_setzero_si128( );
    int sum[4];
    
    for(int i = 0; i < (n/4)*4; i += 4){
        elem_ld = _mm_loadu_si128( a+i );
        sum_vector = _mm_add_epi32( elem_ld, sum_vector);
    }
    
    _mm_storeu_si128( sum, sum_vector );
...
int sum_vectorized_unrolled( int n, int *a )
{
    ...
    __m128i elem_ld = __mm_setzero_si128( );
    __m128i sum_vector = __mm_setzero_si128( );
    int sum[4];
    
    for(int i = 0; i < (n/4)*4; i++){
        elem_ld = __mm_loadu_si128( a+i );
        sum_vector
    }
