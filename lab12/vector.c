...
void vector_set(vector_t *v, size_t loc, int value) {
	...
	/* ADD CODE HERE */
    if(v->size < loc + 1){
        // Allocating a new temporary vector
        vector_t* temp;
        temp = malloc(1 * sizeof(vector_t));
        if(temp == NULL)
            allocation_failed();
        
        // Allocating data portion of temp vector
        temp->data = malloc((loc + 1) * sizeof(int));
        if(temp->data == NULL){
            free(temp);
            allocation_failed();
        }
        ...
        temp->size = loc + 1;
        memcpy(temp->data, v->data, sizeof(int) * v->size);
        ...
        // Freeing old vector
        free(v->data);
        //delete_v(v);          // Why this is not working?
        //printf("size of temp vector: %d\n", temp->size);
        
        // Assigning old vector to new vector
        v->size = temp->size;
        v->data = temp->data;
